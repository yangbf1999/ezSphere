...
pub async fn check_app_update_available(app: AppHandle) -> Result<Option<String>, String> {
    // endpoints 为空时直接返回 None（无更新可用），避免网络错误
    let updater_config = read_updater_config();
    if updater_config.endpoints.is_empty() {
        log::info!("updater endpoints 未配置，跳过检查更新");
        return Ok(None);
    }
    let updater = app
        .updater_builder()
...
/// 读取 tauri.conf.json 里的 updater.endpoints，避免网络请求未配置端点时返回错误
fn read_updater_config() -> UpdaterConfig {
    let manifest_dir = std::env::var("TAURI_CONFIG_DIR")
        .or_else(|_| std::env::var("CARGO_MANIFEST_DIR").map(|p| p))
        .unwrap_or_else(|_| ".".to_string());
    let path = std::path::PathBuf::from(manifest_dir).join("tauri.conf.json");
    if let Ok(content) = std::fs::read_to_string(&path) {
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
            let endpoints = json
                .get("plugins")
                .and_then(|p| p.get("updater"))
                .or_else(|| json.get("updater"))
                ...
                .unwrap_or_default();
            return UpdaterConfig { endpoints };
        }
    }
