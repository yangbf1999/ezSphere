...
/// 从备份恢复 Skill
///
/// 恢复后默认启用 claude / codex / hermes，`current_app` 保留仅为兼容旧前端，已忽略。
#[tauri::command]
pub fn restore_skill_backup(
    ...
    SkillService::restore_from_backup(&app_state.db, &backup_id).map_err(|e| e.to_string())
}
...
/// 从 ZIP 文件安装 Skills
///
/// 安装后默认启用 claude / codex / hermes，`current_app` 保留仅为兼容旧前端，已忽略。
#[tauri::command]
pub fn install_skills_from_zip(
    ...
    SkillService::install_from_zip(&app_state.db, path).map_err(|e| e.to_string())
}
