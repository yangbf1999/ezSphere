...
fn build_tool_search_paths(tool: &str) -> Vec<std::path::PathBuf> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if tool == "hermes" {
            if let Some(local_data) = dirs::data_local_dir() {
                // 官方 installer (install.ps1) 把 CLI 装到
                // %LOCALAPPDATA%\hermes\hermes-agent\venv\Scripts\hermes.exe。
                // 优先于通用 Python Scripts 目录加入搜索路径，让原生安装被
                // 首先探测到（否则没进 PATH 时 probe 会漏报"未安装"）。
                push_unique_path(
                    &mut search_paths,
                    local_data
                        .join("hermes")
...
///
/// 判定顺序（命中即返回）：
/// ① Hermes → `<bin_path 绝对> update`;Hermes CLI 自己知道安装环境,避免 ezsphere
...
echo "[ezsphere] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[ezsphere] Command exited. Press Enter to close."
read -r _
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let content = format!(
            "@echo off\r\necho [ezsphere] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [ezsphere] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
        );
        std::fs::write(&bat_file, &content).map_err(|e| format!("写入批处理文件失败: {e}"))?;
...
mod tests {
    ...
    mod anchored_upgrade {
        ...
        fn hermes_uses_cli_update_anchor() {
            // Hermes 自带 `hermes update`;锚定到命令行默认那处 CLI,避免 ezsphere 猜
    ...
    mod install_strategy {
        ...
        fn hermes_install_uses_official_installer() {
            // Hermes 官方 installer 会处理 Python 3.11+/uv 等运行时;不要再从 ezsphere
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("ezsphere-missing-{unique}"));
