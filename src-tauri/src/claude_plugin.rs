...
mod tests {
    ...
    // is_managed_config 是纯函数，可直接测；其余函数走 dirs::home_dir()，
    // 在 Windows 上无法用 env 可靠隔离，故不测（避免污染真实 ~/.claude）。

    #[test]
    fn is_managed_detects_primary_api_key_any() {
        let content = r#"{"primaryApiKey":"any","other":"x"}"#;
        assert!(is_managed_config(content));
    }

    #[test]
    fn is_managed_rejects_other_api_key_values() {
        assert!(!is_managed_config(r#"{"primaryApiKey":"sk-real-key"}"#));
        assert!(!is_managed_config(r#"{"primaryApiKey":""}"#));
    }

    #[test]
    fn is_managed_rejects_missing_primary_api_key() {
        assert!(!is_managed_config(r#"{"other":"x"}"#));
        assert!(!is_managed_config(r#"{}"#));
    }

    #[test]
    fn is_managed_rejects_non_string_primary_api_key() {
        assert!(!is_managed_config(r#"{"primaryApiKey":123}"#));
        assert!(!is_managed_config(r#"{"primaryApiKey":null}"#));
        assert!(!is_managed_config(r#"{"primaryApiKey":true}"#));
    }

    #[test]
    fn is_managed_rejects_invalid_json() {
        assert!(!is_managed_config("not json"));
        assert!(!is_managed_config("{broken"));
        assert!(!is_managed_config(""));
    }

    #[test]
    fn is_managed_handles_array_json() {
        // 合法 JSON 但非对象 -> 无 primaryApiKey 字段 -> false
        assert!(!is_managed_config(r#"["primaryApiKey","any"]"#));
    }
}
