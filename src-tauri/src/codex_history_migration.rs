...
const LEGACY_CC_SWITCH_CODEX_MODEL_PROVIDER_ID: &str = "ezsphere";
...
/// 迁移前 jsonl / state DB 均备份到 `~/.ezsphere/backups/codex-official-history-unify-v1/`。
...
"#
            ...
            Provider::with_id(
                "legacy-ezsphere".to_string(),
                "Legacy ezSphere".to_string(),
                serde_json::json!({
                    "auth": {},
                    "config": r#"model_provider = "ezsphere"

[model_providers.ezsphere]
name = "AIHubMix"
base_url = "https://aihubmix.example/v1"
"#
                }),
...
"#
        ...
        assert_eq!(
            source_provider_ids,
            source_ids(&["aihubmix", "ezsphere", "rightcode"])
        );
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"ezsphere\"}}\n",
        ...
        assert_eq!(
            ...
            source_ids(&["legacy-ezsphere", "rightcode"])
        );
        ...
        let ezsphere_config: toml::Value =
            toml::from_str(&config_provider_id("legacy-ezsphere")).expect("parse ezsphere config");
        assert_eq!(
            ezsphere_config
                .get("model_provider")
                .and_then(|value| value.as_str()),
        ...
        assert!(ezsphere_config
            .get("model_providers")
            .and_then(|value| value.get("ezsphere"))
            .is_none());
...
"#
    ...
    #[test]
    fn collects_legacy_ezsphere_provider_id_from_stored_config() {
        let db = Database::memory().expect("memory db");
        let mut provider = Provider::with_id(
            "generated-uuid".to_string(),
            ...
            serde_json::json!({
                "auth": {},
                "config": "model_provider = \"ezsphere\"\n\n[model_providers.ezsphere]\nname = \"AIHubMix\"\nbase_url = \"https://aihubmix.example/v1\""
            }),
            None,
        );
        provider.category = Some("aggregator".to_string());
...
"#
    ...
    #[test]
    fn migrates_legacy_ezsphere_provider_template_to_custom() {
        let db = Database::memory().expect("memory db");
        let provider = Provider::with_id(
            "legacy-ezsphere".to_string(),
            "Legacy ezSphere".to_string(),
...
"#
        ...
        assert_eq!(
            ...
            vec!["legacy-ezsphere".to_string()]
        );

        let saved = db
            .get_provider_by_id("legacy-ezsphere", "codex")
            .expect("get provider")
            .expect("provider exists");
        let config_text = saved
            .settings_config
