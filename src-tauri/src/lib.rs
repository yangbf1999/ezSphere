...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("ezsphere://") {
        return false;
    }
...
fn windows_tray_icon() -> Option<Image<'static>> {
    // Use 256px then let Windows downscale. A 16/32px source looks soft on HiDPI.
    const ICON_BYTES: &[u8] = include_bytes!("../icons/tray/windows/tray-256.png");

    match Image::from_bytes(ICON_BYTES) {
        Ok(icon) => Some(icon),
        Err(err) => {
            log::warn!("Failed to load Windows tray icon: {err}");
            None
        }
    }
}
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.ezsphere/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/ezsphere.log）
            {
                ...
                let log_file_path = log_dir.join("ezsphere.log");
            ...
            let db_path = app_config_dir.join("ezsphere.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 ezSphere 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.ezsphere.desktop/applications/ezsphere-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/ezsphere-handler.desktop").exists())
                        .unwrap_or(true);
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first ezsphere:// URL
                        }
                    }
                }
            });
            ...
            // 初始化 deployment Agent 会话映射（Agent 修复循环用）
            {
                use crate::deployment::agent_loop::create_session_map;
                app.manage(create_session_map());
                log::info!("✓ deployment Agent session map initialized");
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // ezSphere deployment/repair commands (ADR-002)
            commands::deployment_commands::deployment_scan_tools,
            commands::deployment_commands::deployment_install_tool,
            commands::deployment_commands::deployment_repair_tool,
            commands::deployment_commands::agent_send_message,
            commands::deployment_commands::agent_abort,
            commands::deployment_commands::agent_reset,
            commands::deployment_commands::deployment_get_install_index,
            commands::deployment_commands::deployment_get_mother_hints,
        ]);
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 ezsphere://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older ezSphere version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including ezsphere.db)\n\
            2) If you see “database version is newer”, please upgrade ezSphere\n\
