...
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "ezsphere-model-catalog.json";
...
/// ezsphere–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// ezsphere–generated file (identified by filename
/// `ezsphere-model-catalog.json`). A user-managed external catalog file is
...
pub fn read_codex_model_catalog_simplified_from_live() -> Result<Option<Value>, AppError> {
    ...
    let Some(catalog_path) = resolve_catalog_path(&config_text, &generated_path) else {
        return Ok(None);
    };
...
/// Given `config.toml` text, resolve the on-disk path of the ezsphere–owned
...
pub(crate) fn resolve_catalog_path(
    ...
    let is_catalog_owned = referenced_path.file_name().and_then(|name| name.to_str())
        == Some(CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME);
    if !is_catalog_owned {
        return None;
    }
...
mod tests {
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"ezsphere-model-catalog.json\"\n";
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/ezsphere-model-catalog.json");
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/ezsphere-model-catalog.json");
        assert!(resolve_catalog_path("", &generated).is_none());
        assert!(
            resolve_catalog_path("model = \"gpt-5\"", &generated).is_none(),
            "no model_catalog_json field should yield None"
        );
    }

    #[test]
    fn resolve_catalog_path_accepts_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/ezsphere-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/ezsphere-model-catalog.json"
"#;
        let resolved = resolve_catalog_path(config, &generated).expect("path resolves");
...
"#;
        assert!(
            resolve_catalog_path(config, &generated).is_none(),
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as ezsphere would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\ezsphere-model-catalog.json");
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/ezsphere-model-catalog.json");
    ...
    #[test]
    fn set_catalog_json_none_removes_catalog_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/ezsphere-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove ezsphere-owned field regardless of path format"
        );
    }
...
model_catalog_json = "ezsphere-model-catalog.json"
"#;
        let generated_path = PathBuf::from("/home/user/.codex/ezsphere-model-catalog.json");
        let result = resolve_catalog_path(config_text, &generated_path);
        assert_eq!(
            result,
            Some(generated_path),
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by ezsphere"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "ezsphere-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative ezsphere-owned field"
        );
    }
}
