...
fn restore_live_settings_for_provider_backfill(
    ...
    // `modelCatalog` is a ezsphere–private field whose SSOT is the DB. Live's
    ...
    match app_type {
        AppType::Claude => {
            ...
            let provider_settings =
                sanitize_claude_settings_for_live(&provider.settings_config);

            // 合并现有 settings.json，保留非供应商字段
            // (如 permissions、mcpServers 等 Claude Code 自身写入的配置)。
            // 否则首次安装后启用供应商时，全量覆盖会清空 claudecode 的默认设置，
            // 用户看到的就是 "settings.json 被重置"。
            // 策略：替换供应商字段（env 等），保留其他顶层字段。
            // 与 Gemini 的 write_gemini_live 对齐。
            let mut merged: Value = if path.exists() {
            ...
            if let Some(merged_obj) = merged.as_object_mut() {
                // 先清除旧供应商的 env，避免切到官方供应商时残留旧配置
                merged_obj.remove("env");
                // 写入新供应商字段
                if let Value::Object(provider_obj) = provider_settings {
                    for (key, value) in provider_obj {
                        merged_obj.insert(key, value);
                    }
                }
            } else {
                // 现有文件不是 object（极少），退回全量替换
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a ezsphere private field that lives only in
            // the DB SSOT plus the `ezsphere-model-catalog.json` projection
...
/// into the ezSphere database. Each provider found will be added to the
