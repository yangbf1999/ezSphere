...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.ezsphere/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.ezsphere/skill-backups/）
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        // 默认路径：回退到用户主目录下的标准位置（测试可通过 CC_SWITCH_TEST_HOME 覆盖）
    ...
    pub fn restore_from_backup(
        ...
        restored_skill.apps = Self::default_skill_install_apps();
        ...
        for app in [AppType::Claude, AppType::Codex, AppType::Hermes] {
        ...
        log::info!(
            "Skill {} 已从备份恢复到 {}，已启用 claude, codex, hermes",
            restored_skill.name,
            restore_path.display()
        );

        if let Err(err) = Self::delete_backup(backup_id) {
            log::warn!(
                "Skill {} 已恢复，但删除备份 {} 失败: {err:#}",
                restored_skill.name,
                backup_id
            );
        }
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 ezSphere 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "ezsphere".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 ezSphere 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "ezsphere".to_string()));
        ...
        for selection in imports {
            ...
            if let Err(err) =
                Self::sync_imported_skill_to_enabled_apps(&skill.directory, &skill.apps)
            {
    ...
    /// 将导入的技能同步到用户启用的应用目录。
    ///
    /// 若目标应用下已存在真实目录（通常是导入来源），则跳过，避免覆盖用户原有文件。
    fn sync_imported_skill_to_enabled_apps(directory: &str, apps: &SkillApps) -> Result<()> {
        for app in AppType::all() {
            ...
            if dest.exists() && !Self::is_symlink(&dest) {
                log::debug!(
                    "Skip syncing imported skill {directory} to {app:?}: destination already exists"
                );
                continue;
            }
    ...
    /// ZIP 安装 / 备份恢复后默认启用的应用（忽略调用方传入的 currentApp）
    fn default_skill_install_apps() -> SkillApps {
        SkillApps {
            claude: true,
            codex: true,
            hermes: true,
    ...
    /// 3. 复制到 SSOT 并保存到数据库
    /// 4. 同步到 claude / codex / hermes 应用目录
    pub fn install_from_zip(
        ...
        for skill_dir in skill_dirs {
            ...
            // 创建 InstalledSkill 记录
            let apps = Self::default_skill_install_apps();
            let skill = InstalledSkill {
            ...
            log::info!(
                "Skill {} installed from ZIP, enabled for claude, codex, hermes",
                skill.name,
            );
