...
///
/// 原因：接管模式下 `*_MODEL` 必须由 ezSphere 写成稳定的 Claude 角色别名，
                                ...
                                match env_obj {
                                    ...
                                    None => {
                                        ...
                                        if let Some(root) = provider.settings_config.as_object_mut()
                                        {
                                            ...
                                            log::warn!(
                                                "Claude model settings_config 格式异常（非对象），跳过写入 Token (provider: {provider_id})"
                                            );
                                        }
                                    }
                                }
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "ezsphere-model-catalog.json must be created on provider switch"
        );
...
"#;
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the ezsphere generated
        ...
        assert!(
            ...
            "restored pointer must still reference the ezsphere generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the ezsphere catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the ezsphere catalog file"
        );
        assert!(
