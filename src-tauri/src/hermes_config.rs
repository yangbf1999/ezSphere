...
/// 默认路径:
/// - macOS/Linux: `~/.hermes/`
/// - Windows: `~\AppData\Local\hermes\`（Python Hermes 应用的默认路径）
/// 可通过 settings.hermes_config_dir 覆盖
///
/// 注意：当 CC_SWITCH_TEST_HOME 环境变量被设置（测试场景）时，所有平台
/// 统一使用测试目录下的 .hermes，以确保测试隔离。
pub fn get_hermes_dir() -> PathBuf {
    if let Some(override_dir) = get_hermes_override_dir() {
        return override_dir;
    }

    // 测试场景：CC_SWITCH_TEST_HOME 设置时，所有平台统一用 ~/.hermes
    ...
    {
        // Python Hermes 应用在 Windows 上使用 %LOCALAPPDATA%\hermes
        // (AppData\Local)。必须用 data_local_dir() 而非 data_dir()——后者在
        // Windows 上是 %APPDATA% (AppData\Roaming)，与 Hermes 实际的安装/配置
        // 目录不一致，会让 ezSphere 与 Hermes 各自读写不同的 config.yaml。
        if let Some(data_dir) = dirs::data_local_dir() {
            return data_dir.join("hermes");
        }
    }
...
fn write_yaml_section_to_config_locked(
    ...
    // 用完整 YAML 解析-修改-序列化替换字符串替换逻辑，
    // 避免现有 config.yaml 有畸形内容时越改越坏（#fix-hermes-config-corruption）。
    // 兼容 Windows \r\n 行尾。
    let normalized = raw.replace("\r\n", "\n");
    let mut doc: serde_yaml::Value = if normalized.trim().is_empty() {
        serde_yaml::Value::Mapping(Default::default())
    } else {
        serde_yaml::from_str(&normalized).unwrap_or_else(|e| {
            log::warn!(
                "Hermes config parse failed (will append section): {e}"
            );
            serde_yaml::Value::Mapping(Default::default())
        })
    };

    if let serde_yaml::Value::Mapping(ref mut mapping) = doc {
        mapping.insert(
            serde_yaml::Value::String(section_key.to_string()),
            value.clone(),
        );
    } else {
        // 根节点不是 mapping（罕见），退回字符串追加
        let mut result = if !normalized.is_empty() && !normalized.ends_with('\n') {
            format!("{normalized}\n")
        } else {
            normalized
        };
        let mut m = serde_yaml::Mapping::new();
        m.insert(serde_yaml::Value::String(section_key.to_string()), value.clone());
        let serialized = serde_yaml::to_string(&serde_yaml::Value::Mapping(m))
            .map_err(|e| AppError::Config(format!("Failed to serialize YAML section: {e}")))?;
        result.push_str(&serialized);
        if !result.ends_with('\n') {
            result.push('\n');
        }
...
///
/// Input (from ezSphere UI / database):
/// ```json
...
/// `providers:` dict. ezSphere treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by ezSphere)
...
/// keeping ezSphere free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay ezSphere must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a ezSphere edit to an unrelated field would silently
...
// ezSphere fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later ezSphere edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a ezSphere toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
