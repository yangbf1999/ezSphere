...
pub const TRAY_ID: &str = "ezsphere";
...
/// 对供应商列表排序：created_at（新在前）→ sort_index → name
fn sort_providers(
    ...
    sorted.sort_by(|(_, a), (_, b)| {
        match (a.created_at, b.created_at) {
            (Some(time_a), Some(time_b)) if time_a != time_b => {
...
pub fn handle_tray_menu_event(app: &tauri::AppHandle, event_id: &str) {
    ...
    match event_id {
        ...
        "open_website" => {
            if let Err(e) = app.opener().open_url("https://ezsphere.io", None::<String>) {
