...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    11 => {
                        log::info!("迁移数据库从 v11 到 v12（添加 installed_tools 表）");
                        Self::migrate_v11_to_v12(conn)?;
                        Self::set_user_version(conn, 12)?;
                    }
                    12 => {
                        log::info!("迁移数据库从 v12 到 v13（默认代理端口 15721 → 15731，避免与 ezSphere 冲突）");
    ...
    fn migrate_v2_to_v3(conn: &Connection) -> Result<(), AppError> {
        ...
        // 说明：v3 结构将 Skills 的 SSOT 迁移到 ~/.ezsphere/skills/，
    ...
    /// v11 -> v12：添加 installed_tools 表，记录已安装工具状态
    fn migrate_v11_to_v12(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!("v11 -> v12 创建 installed_tools 表失败: {e}")))?;

        log::info!("v11 -> v12 迁移完成：installed_tools 表已创建");
        Ok(())
    }

    /// v12 -> v13 迁移：默认代理端口 15721 → 15731（避免与 ezSphere 冲突）
    fn migrate_v12_to_v13(conn: &Connection) -> Result<(), AppError> {
        // 安全检查：proxy_config 表可能不存在（旧版 schema 测试场景），此时跳过迁移而不报错。
        let table_exists: bool = conn
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name='proxy_config')",
                [],
                |row| row.get(0),
            )
            .unwrap_or(false);
        ...
        if table_exists {
            // 同时检查 listen_port 列是否存在（旧版 schema 可能没有此列）
            let col_exists: bool = conn
                .query_row(
                    "SELECT EXISTS(SELECT 1 FROM pragma_table_info('proxy_config') WHERE name='listen_port')",
                    [],
                    |row| row.get(0),
                )
                .unwrap_or(false);
            if col_exists {
                conn.execute(
                    "UPDATE proxy_config SET listen_port = 15731 WHERE listen_port = 15721",
                    [],
                )
                .map_err(|e| AppError::Database(format!("v12 -> v13 更新 listen_port 失败: {e}")))?;
            }
        }

        log::info!("v12 -> v13 迁移完成：proxy_config listen_port 已更新为 15731");
        Ok(())
    }
