...
#[test]
#[ignore = "skill sync dir detection may need ezsphere brand update"]
...
#[test]
fn import_from_apps_syncs_to_newly_enabled_app_directories() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();

    let claude_skill_dir = home.join(".claude").join("skills").join("shared-skill");
    write_skill(&claude_skill_dir, "Shared Skill");
    fs::write(claude_skill_dir.join("prompt.md"), "from claude").expect("write claude prompt");
    ...
    assert_eq!(imported.len(), 1, "expected exactly one imported skill");
    assert!(
        imported[0].apps.claude && imported[0].apps.codex,
        "import should preserve selected app flags"
    );
    assert!(
        codex_skills_dir.join("shared-skill").join("SKILL.md").exists(),
        "import should sync the skill into newly enabled Codex skills dir"
    );
    assert_eq!(
        fs::read_to_string(claude_skill_dir.join("prompt.md")).expect("read claude prompt"),
        "from claude",
        "import should not overwrite the existing Claude source directory"
    );
}
...
fn import_from_apps_does_not_rewrite_selected_app_directory() {
    ...
    let ssot_skill_dir = home.join(".ezsphere").join("skills").join("codex-skill");
...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    let ssot_dir = home.join(".ezsphere").join("skills");
...
fn uninstall_skill_creates_backup_before_removing_ssot() {
    ...
    let ssot_skill_dir = home.join(".ezsphere").join("skills").join("backup-skill");
...
fn restore_skill_backup_restores_files_to_ssot_and_current_app() {
    ...
    let ssot_skill_dir = home.join(".ezsphere").join("skills").join("restore-skill");
    ...
    let restored = SkillService::restore_from_backup(&state.db, &backup_id)
    ...
    assert!(restored.apps.hermes, "restored skill should enable Hermes");
    assert!(
        !restored.apps.gemini && !restored.apps.opencode,
        "restore should only enable claude, codex, and hermes"
    );
    assert!(
        home.join(".ezsphere")
            .join("skills")
    ...
    assert!(
        home.join(".codex")
            ...
            .join("prompt.md")
            .exists(),
        "restored skill should sync to Codex"
    );
    assert!(
        home.join(".claude")
            .join("skills")
            ...
            .join("prompt.md")
            .exists(),
        "restored skill should sync to Claude"
    );
    assert!(
        state
            .db
    ...
    assert!(
        ...
        "backup should be removed after a successful restore"
    );
}
