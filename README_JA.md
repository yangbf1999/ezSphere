...
1. **プロバイダ追加**：「モデルセンター」ページに入る -> プリセットを選ぶかカスタム設定を作成
2. **プロバイダ切り替え**：
   - アプリマネージャー：プロバイダを選択 -> 「有効化」をクリック
   - システムトレイ：プロバイダ名を直接クリック（即時反映）
3. **反映方式**：ターミナルまたは対応する CLI ツールを再起動して変更を適用（Claude Code は再起動不要）
4. **公式ログインに戻す**：「公式ログイン」プリセットを追加し、CLI ツールを再起動後にログイン/OAuth フローに従う

### MCP、Prompts、Skills とセッション

- **MCP**：「MCP管理」ページに入り、「MCP」ボタンをクリック -> テンプレートまたはカスタム設定でサーバーを追加 -> アプリごとの同期トグル
- **Prompts**：「プロンプト管理」ページに入る -> Markdown エディタでプリセットを作成 -> 有効化して live ファイルに同期
- **Skills**：「スキル管理」ページに入る -> ローカルからスキルをインポート -> 対応アプリにワンクリックインストール
- **セッション**：「セッション管理」ページに入る -> 対応するセッションソースを閲覧、検索、復元

> **注意**：初回起動時に、既存の CLI ツール設定を手動でインポートしてデフォルトプロバイダとして使用できます。
