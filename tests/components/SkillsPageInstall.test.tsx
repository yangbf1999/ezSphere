...
const removeRepoMutateAsyncMock = vi.fn();
...
vi.mock("@/hooks/useSkills", () => ({
  ...
  useRemoveSkillRepo: () => ({
    mutateAsync: removeRepoMutateAsyncMock,
  }),
...
describe("SkillsPage - skills.sh install (regression)", () => {
  beforeEach(() => {
    ...
    refetchDiscoverableMock.mockResolvedValue({ data: [] });
    refetchReposMock.mockReset();
    refetchReposMock.mockResolvedValue({ data: [] });
    addRepoMutateAsyncMock.mockReset();
    addRepoMutateAsyncMock.mockResolvedValue({});
    removeRepoMutateAsyncMock.mockReset();
    removeRepoMutateAsyncMock.mockResolvedValue({});
    searchCache.clear();
  });
  ...
  it("executes header actions and manages repositories through the page", async () => {
    skillReposMock = [makeSkillRepo()];
    discoverableSkillsMock = [makeDiscoverableSkill()];
    const ref = createRef<SkillsPageHandle>();
    render(<SkillsPage ref={ref} initialApp="claude" />);
    ...
    act(() => {
      for (const action of getSkillsPageHeaderActions("repos")) {
        action.execute(ref.current);
      }
    });
    expect(refetchDiscoverableMock).toHaveBeenCalled();
    expect(refetchReposMock).toHaveBeenCalled();

    const user = userEvent.setup();
    const repoInput = await screen.findByPlaceholderText(
      "skills.repo.urlPlaceholder",
    );
    await user.type(repoInput, "new-owner/new-repo");
    await user.click(screen.getByText("skills.repo.add"));
    await waitFor(() => expect(addRepoMutateAsyncMock).toHaveBeenCalled());

    await user.click(
      screen.getByRole("button", {
        name: "common.delete owner-a/repo-a",
      }),
    );
    await waitFor(() =>
      expect(removeRepoMutateAsyncMock).toHaveBeenCalledWith({
        owner: "owner-a",
        name: "repo-a",
      }),
    );
