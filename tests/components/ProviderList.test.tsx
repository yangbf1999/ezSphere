  ...
  ProviderCard: (props: any) => {
    ...
    return (
      <div data-testid={`provider-card-${provider.id}`}>
        ...
        <button onClick={() => onRemoveFromConfig?.(provider)}>remove-config</button>
        <button onClick={() => onDisableOmo?.()}>disable-omo</button>
        <button onClick={() => onDisableOmoSlim?.()}>disable-omo-slim</button>
        <button onClick={() => onOpenWebsite(provider.websiteUrl ?? "https://example.test")}>website</button>
        <button onClick={() => onOpenTerminal?.(provider)}>terminal</button>
        <button onClick={() => onTest?.(provider)}>test-provider</button>
        <button onClick={() => onToggleFailover(true)}>failover-on</button>
        <button onClick={() => onToggleFailover(false)}>failover-off</button>
        <button onClick={() => onSetAsDefault?.()}>set-default</button>
...
describe("ProviderList Component", () => {
  it("should render skeleton placeholders when loading", () => {
    ...
    const placeholders = container.querySelectorAll(".prov-skeleton");
    expect(placeholders).toHaveLength(3);
  });

  it("should show the empty list state when no providers exist", () => {
    ...
    expect(container.querySelector(".prov-empty")).toBeInTheDocument();
  ...
  it("should render in order returned by useDragSort and pass through action callbacks", () => {
    ...
    const handleDisableOmo = vi.fn();
    const handleDisableOmoSlim = vi.fn();
    const handleOpenTerminal = vi.fn();
    const handleSetAsDefault = vi.fn();
    ...
    expect(handleRemoveFromConfig).toHaveBeenCalledWith(providerB);
    expect(handleDisableOmo).toHaveBeenCalled();
    expect(handleDisableOmoSlim).toHaveBeenCalled();
    expect(handleOpenWebsite).toHaveBeenCalled();
    expect(handleOpenTerminal).toHaveBeenCalledWith(providerB);
    expect(handleSetAsDefault).toHaveBeenCalledWith(providerB);
  ...
  it("filters providers with the controlled search term", () => {
