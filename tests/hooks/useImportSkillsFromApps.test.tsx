...
describe("mergeImportedSkills", () => {
  ...
  it("appends newly imported skills without dropping existing unrelated ones", () => {
    ...
    expect(merged.map((s) => s.id)).toEqual(["skill-a", "skill-b"]);
  });
});
...
describe("sortInstalledSkills", () => {
  it("sorts skills by name ascending", () => {
    const sorted = sortInstalledSkills([
      makeSkill({ id: "z", name: "Zeta" }),
      makeSkill({ id: "a", name: "Alpha" }),
      makeSkill({ id: "m", name: "Middle" }),
    ]);
    expect(sorted.map((s) => s.name)).toEqual(["Alpha", "Middle", "Zeta"]);
  });
});
