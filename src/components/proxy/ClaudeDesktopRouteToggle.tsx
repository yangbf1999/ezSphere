...
export function ClaudeDesktopRouteToggle({
  ...
  const tooltipText = isRunning
    ...
    : t("claudeDesktop.route.tooltip.inactive", {
        address: routeAddress,
        port: routePort,
        defaultValue: `开启 Claude Desktop 本地路由，用于需要模型映射或格式转换的模型。当前配置地址：${routeAddress}:${routePort}`,
      });
  ...
  return (
    ...
    >
      {isBusy ? (
        ...
        <Radio
          className={cn("prov-toggle-icon", isRunning && "is-active animate-pulse")}
        />
      )}
      <Switch
        checked={isRunning}
        ...
        disabled={isBusy}
        aria-label={t("claudeDesktop.route.switchLabel", {
          defaultValue: "Claude Desktop 本地路由",
        })}
      />
    </div>
  );
