    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold">
          {t("universalProvider.title", { defaultValue: "统一API网关" })}
        </h2>
      ...
      <p className="text-sm text-muted-foreground">
        {t("universalProvider.description", {
          defaultValue:
            "统一API网关可以同时管理 Claude、Codex 和 Gemini 的配置。修改后会自动同步到所有启用的应用。",
        })}
      </p>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-12 text-center">
          <Layers className="mb-3 h-10 w-10 text-muted-foreground/50" aria-hidden="true" />
          <p className="text-sm text-muted-foreground">
            {t("universalProvider.empty", {
              defaultValue: "还没有统一API网关",
            })}
          </p>
          <p className="mt-1 text-xs text-muted-foreground/70">
            {t("universalProvider.emptyHint", {
              defaultValue: "点击下方「添加统一API网关」按钮创建一个",
            })}
          </p>
        </div>
      ) : (
      ...
      <ConfirmDialog
        ...
        message={t("universalProvider.syncConfirmDescription", {
          defaultValue: `同步 "${syncConfirm.name}" 将会覆盖 Claude、Codex 中关联的API网关配置。确定要继续吗？`,
