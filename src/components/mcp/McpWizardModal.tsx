...
const McpWizardModal: React.FC<McpWizardModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent
        className="[&_.ez-modal-content__inner]:max-h-[90vh]"
        zIndex="alert"
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <div className="space-y-4 min-h-[400px]">
            ...
            {/* Title */}
            <div>
              <label htmlFor="mcp-wizard-title" className="mb-1 block text-sm font-medium text-foreground">
            ...
            {wizardType === "stdio" && (
              <>
                {/* Command */}
                <div>
                  <label htmlFor="mcp-wizard-command" className="mb-1 block text-sm font-medium text-foreground">
                ...
                {/* Args */}
                <div>
                  <label htmlFor="mcp-wizard-args" className="mb-1 block text-sm font-medium text-foreground">
                    {t("mcp.wizard.args")}
                  </label>
                  <textarea
                    id="mcp-wizard-args"
                    ...
                    rows={3}
                    className="w-full rounded-md border border-border-default bg-background px-3 py-2 text-sm font-mono text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 resize-y"
                  />
                </div>

                {/* Env */}
                <div>
                  <label htmlFor="mcp-wizard-env" className="mb-1 block text-sm font-medium text-foreground">
                    {t("mcp.wizard.env")}
                  </label>
                  <textarea
                    id="mcp-wizard-env"
            ...
            {(wizardType === "http" || wizardType === "sse") && (
              <>
                {/* URL */}
                <div>
                  <label htmlFor="mcp-wizard-url" className="mb-1 block text-sm font-medium text-foreground">
                    {t("mcp.wizard.url")}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <Input
                ...
                {/* Headers */}
                <div>
                  <label htmlFor="mcp-wizard-headers" className="mb-1 block text-sm font-medium text-foreground">
        ...
        <DialogFooter className="flex gap-2 border-t-0 bg-transparent pt-2 sm:justify-end">
          ...
          <Button variant="btn-primary" onClick={handleApply}>
            {t("mcp.wizard.apply")}
          </Button>
        </DialogFooter>
      </DialogContent>
