...
const DEFAULT_REMOTE_ROOT = "ezsphere-sync";
const LEGACY_REMOTE_ROOT = "ezsphere-sync";

function normalizeRemoteRoot(value?: string | null): string {
  const trimmed = value?.trim();
  if (!trimmed || trimmed === LEGACY_REMOTE_ROOT) {
...
function buildPasswordPreservationKey(values: {
  ...
  return JSON.stringify({
    ...
    remoteRoot: normalizeRemoteRoot(values.remoteRoot),
...
export function WebdavSyncSection({
  ...
  const [s3RemoteRoot, setS3RemoteRoot] = useState(
    normalizeRemoteRoot(s3Config?.remoteRoot),
  );
  ...
  useEffect(() => {
    ...
    setForm(() => {
      ...
      const nextRemoteRoot = normalizeRemoteRoot(config.remoteRoot);
  ...
  useEffect(() => {
    ...
    setS3RemoteRoot(normalizeRemoteRoot(s3Config.remoteRoot));
  ...
  const buildSettings = useCallback((): WebDavSyncSettings | null => {
    ...
    return {
      ...
      remoteRoot: normalizeRemoteRoot(form.remoteRoot),
  ...
  const buildS3Settings = useCallback((): S3SyncSettings => {
    return {
      ...
      remoteRoot: normalizeRemoteRoot(s3RemoteRoot),
  ...
  const currentRemotePath = `/${normalizeRemoteRoot(form.remoteRoot)}/v2/db-v6/${form.profile.trim() || "default"}`;
  const currentS3RemotePath = `${s3Bucket.trim() || "bucket"}/${normalizeRemoteRoot(s3RemoteRoot)}/v2/db-v6/${s3Profile.trim() || "default"}`;
      ...
      {syncType === "webdav" && (
        <div className="space-y-4 rounded-lg border border-border bg-muted/40 p-6">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            {dirty && (
              <span className="settings-status-inline settings-status-inline-warning animate-in fade-in duration-200">
                <span className="settings-status-dot" />
                {t("settings.webdavSync.unsaved")}
              </span>
            )}
            {!dirty && justSaved && (
              <span className="settings-status-inline settings-status-inline-success animate-in fade-in duration-200">
          ...
          <DialogBody className="pt-0">
            <DialogDescription asChild>
              <div className="space-y-3 text-sm leading-relaxed">
                ...
                {remoteInfo && remoteIsLegacy && (
                  <p className="font-medium settings-token-warning">
