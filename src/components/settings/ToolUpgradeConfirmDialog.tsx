      ...
      <DialogContent className="max-w-md" zIndex="alert">
        <DialogHeader className="space-y-2 border-b-0 bg-transparent pb-0">
          <DialogTitle className="flex items-center gap-2 text-base font-semibold">
            <AlertTriangle className="h-5 w-5 settings-token-warning" aria-hidden="true" />
        ...
        <div className="max-h-[50vh] space-y-3 overflow-y-auto">
          {plans.map((plan) => (
            <div
              key={plan.tool}
              className="settings-alert-warning space-y-1.5 p-2.5"
            >
              <div className="text-xs font-medium">
                {displayName(plan.tool)}
              </div>
              {!plan.anchored && (
                <div className="settings-alert-detail text-[10px] leading-snug">
