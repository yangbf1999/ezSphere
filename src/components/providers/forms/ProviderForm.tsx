...
import {
  filterPresetEntriesForModelsDirectory,
...
import {
  ADDITIVE_PROVIDER_KEY_PATTERN,
  AdditiveProviderKeyField,
  getAdditiveProviderKeyError,
} from "./AdditiveProviderKeyField";
...
function ProviderFormFull({
  ...
  const presetEntries = useMemo(() => {
    if (appId === "codex") {
      return filterPresetEntriesForModelsDirectory(
        codexProviderPresets,
        "codex",
      );
    ...
    return filterPresetEntriesForModelsDirectory(providerPresets, "claude");
  }, [appId]);
  ...
  // 新建只比 DB：Hermes/OpenCode live 里有大量内置项，算进去会误报重名并导致输入闪烁。
  // 编辑改名才并入 live（与后端 rename 冲突检查一致）。
  const additiveExistingProviderKeys = useMemo(() => {
    const mergeKeys = (dbKeys: string[], liveIds: string[]) =>
      Array.from(
        new Set(
          (isEditMode ? [...dbKeys, ...liveIds] : dbKeys).filter(
            (key) => Boolean(key) && key !== providerId,
          ),
        ),
      );

    if (appId === "opencode" && !isAnyOmoCategory) {
      return mergeKeys(existingOpencodeKeys, opencodeLiveProviderIds);
    }
    if (appId === "openclaw") {
      return mergeKeys(
    ...
    if (appId === "hermes") {
      return mergeKeys(hermesForm.existingHermesKeys, hermesLiveProviderIds);
    }
    return [];
  }, [
    appId,
  ...
  const handleSubmit = async (values: ProviderFormData) => {
    ...
    if (!values.name.trim()) {
      issues.push(
        fromModelsCenter
          ? t("models.fillName", { defaultValue: "请填写名称" })
    ...
    // 错误只 toast，不在输入框下方渲染（点添加会 blur，下方红字会闪一下）
    if (appId === "opencode" && !isAnyOmoCategory) {
      const opencodeKey = opencodeForm.opencodeProviderKey.trim();
      if (!opencodeKey) {
        toast.error(t("opencode.providerKeyRequired"));
        return;
      }
      if (!ADDITIVE_PROVIDER_KEY_PATTERN.test(opencodeKey)) {
    ...
    if (appId === "openclaw") {
      const openclawKey = openclawForm.openclawProviderKey.trim();
      if (!openclawKey) {
        toast.error(t("openclaw.providerKeyRequired"));
        return;
      }
      if (!ADDITIVE_PROVIDER_KEY_PATTERN.test(openclawKey)) {
    ...
    if (appId === "hermes") {
      const hermesKey = hermesForm.hermesProviderKey.trim();
      if (!hermesKey) {
        toast.error(t("hermes.form.providerKeyRequired"));
        return;
      }
      if (!ADDITIVE_PROVIDER_KEY_PATTERN.test(hermesKey)) {
  ...
  const performSubmit = async (
    ...
    // Always persist category (incl. "custom"); otherwise Models grid
    // domestic-only filter hides entries with category !== "custom".
    if (category) {
    ...
    const nextMeta: ProviderMeta = {
      ...
      // 保存 providerType（用于识别 Copilot / Codex OAuth 等特殊模型）
      providerType,
  ...
  useEffect(() => {
    if (initialData || !initialPresetId) return;
    handlePresetChange(initialPresetId);
  }, [initialData, initialPresetId]);
            ...
            beforeNameSlot={
              appId === "opencode" && !isAnyOmoCategory ? (
                <AdditiveProviderKeyField
                  ...
                  hint={t("opencode.providerKeyHint")}
                  lockedHint={t("opencode.providerKeyLockedHint", {
                    defaultValue:
                      "该模型已添加到应用配置中，模型标识不可修改",
                  })}
                  disabled={isProviderKeyLockStateLoading}
                ...
                <AdditiveProviderKeyField
                  ...
                  hint={t("openclaw.providerKeyHint")}
                  lockedHint={t("openclaw.providerKeyLockedHint", {
                    defaultValue:
                      "该模型已添加到应用配置中，模型标识不可修改",
                  })}
                  disabled={isProviderKeyLockStateLoading}
