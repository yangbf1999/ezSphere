...
export function CodexFormFields({
  ...
  return (
    <>
      ...
      {/* 高级选项 —— 本地路由映射/模型映射/思考能力/自定义 UA；预设模型通常无需展开 */}
      {category !== "official" && (
        <Collapsible
        ...
        >
          <CollapsibleTrigger asChild>
            <Button
              type="button"
              variant="native"
              className="inline-flex h-8 w-full items-center justify-start gap-1.5 px-0 text-left text-sm font-medium text-foreground hover:opacity-70"
            >
              {advancedExpanded ? (
          ...
          {!advancedExpanded && (
            <p className="mt-1 ml-1 text-xs text-muted-foreground">
              {t("codexConfig.advancedSectionHint", {
                defaultValue:
                  "包含本地路由映射、模型映射、思考能力与自定义 User-Agent。模型使用 Chat Completions 协议或非 GPT 模型时，需在此开启本地路由映射。",
              })}
            </p>
          )}
          <CollapsibleContent className="space-y-3 pt-3">
            {/* 上游格式 + 本地路由映射 —— 两个平级、相互独立的控件。
                格式不依赖路由：Responses 原生模型无需开启路由即可直连；
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {t("codexConfig.upstreamFormatHint", {
                      defaultValue:
                        "模型原生是 Responses API 就选 Responses（直连，不转换格式）；使用 Chat Completions 协议就选 Chat（转换为 Chat Completions）。",
                    })}
                  </p>
                </div>
                    ...
                    <p className="text-xs leading-relaxed text-muted-foreground">
                      {takeoverEnabled
                        ...
                        : t("codexConfig.localRoutingOffHint", {
                            defaultValue:
                              "模型模型名无需改写、也无需在 /model 菜单展示自定义名称时，可保持关闭；需要模型映射时打开。",
                          })}
                    </p>
                  </div>
                  <Switch
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {t("codexConfig.reasoningSectionHint", {
                      defaultValue:
                        "预设模型已自动配置；自定义模型会按名称/地址自动推断。仅当自动识别不准时才需手动覆盖。",
                    })}
                  </p>
                </div>
            ...
            {/* 模型映射 —— 仅在本地路由开启 + 可编辑时显示（与上游格式解耦，
                Responses 原生模型同样可配置）；上方恒有 UA 字段，分隔线无需条件 */}
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {t("codexConfig.modelMappingHint", {
                      defaultValue:
                        "选择模型角色后，ezSphere 会自动生成 Codex 兼容路由；菜单显示名可以填 DeepSeek、Kimi 等品牌模型，实际请求模型按右侧填写内容发送。",
                    })}
                  </p>
                </div>
