...
export function ClaudeDesktopProviderForm({
  ...
  const [, setSelectedPresetId] = useState<string | null>(
  ...
  const handleSubmit = async (values: ProviderFormData) => {
    ...
    if (isOfficial) {
      // 官方模型使用 Claude Desktop 内置 1P 模式，保持空 env 占位；
        ...
        {isOfficial && (
          <div className="rounded-lg border border-border-default bg-muted/20 p-3 text-sm text-muted-foreground">
            {t("claudeDesktop.officialNotice", {
              defaultValue:
                "Claude Desktop 官方模型使用应用内置的 1P 登录，无需配置 API Key 和接口地址。",
            })}
          </div>
        )}
                  ...
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    {needsModelMapping
                      ? t("claudeDesktop.modelMappingOnHint", {
                          defaultValue:
                            "Claude Desktop 只接受 claude-sonnet-* / claude-opus-* / claude-haiku-* 三档角色 ID。开启后 ezSphere 会把这三档映射到模型的实际模型，并在使用期间保持本地路由开启。",
                        })
                      : t("claudeDesktop.modelMappingOffHint", {
                          defaultValue:
                            "仅当模型直接接受 Claude Desktop 可识别的三档角色 ID（claude-sonnet-* / claude-opus-* / claude-haiku-*）时才适用直连；其他模型名（含 claude-3-5-sonnet-… 等旧式 ID）请打开此开关走映射。",
                        })}
                  </p>
                </div>
                <Switch
                ...
                {!directModelsExpanded && (
                  <p className="ml-1 mt-1 text-xs text-muted-foreground">
                    {t("claudeDesktop.directModelListCollapsedHint", {
                      defaultValue:
                        "原生 Claude 模型模型通常不用填写，Claude Desktop 会自动读取 /v1/models。",
                    })}
                  </p>
                )}
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <p className="flex-1 text-xs leading-relaxed text-muted-foreground">
                        {t("claudeDesktop.directModelListHint", {
                          defaultValue:
                            "仅当模型的 /v1/models 不可用或没有返回 Claude Desktop 可识别的 Sonnet / Opus / Haiku 模型名时填写；勾选 1M 会向 Claude Desktop 声明支持 1M 上下文。",
                        })}
                      </p>
                      {renderActionButtons(
                        () =>
