...
export function ClaudeFormFields({
  ...
  const renderModelInput = (
        ...
        <div className="flex gap-1">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="icon" className="shrink-0" aria-label={t("common.selectModel", { defaultValue: "选择模型" })}>
      ...
      return (
        <div className="flex gap-1">
          <Input
            ...
            className="flex-1"
          />
          <Button variant="outline" size="icon" className="shrink-0" disabled aria-label={t("common.loading", { defaultValue: "加载中" })}>
    ...
    // 普通模型: 使用 ModelInputWithFetch（获取按钮在 section 标题旁）
    return (
      <ModelInputWithFetch
        id={id}
        value={value}
        ...
        <Collapsible open={advancedExpanded} onOpenChange={setAdvancedExpanded}>
          <CollapsibleTrigger asChild>
            <Button
              ...
              variant="native"
              size="lg"
              className="inline-flex h-8 gap-1.5 px-0 text-sm font-medium text-foreground hover:opacity-70"
            >
              {advancedExpanded ? (
                <ChevronDown className="h-4 w-4" />
              ) : (
