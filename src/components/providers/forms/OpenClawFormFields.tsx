      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {t("openclaw.apiProtocolHint", {
            defaultValue:
              "选择与模型 API 兼容的协议类型。大多数模型使用 OpenAI Completions 格式。",
          })}
        </p>
      </div>
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {t("openclaw.userAgentHint", {
              defaultValue: "部分模型需要浏览器 User-Agent 才能正常访问。",
            })}
          </p>
        </div>
        <Switch checked={userAgent} onCheckedChange={onUserAgentChange} aria-label={t("openclaw.userAgent", { defaultValue: "User-Agent" })} />
      </div>
      ...
      <div className="space-y-3">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              ...
                              size="icon"
                              className="shrink-0"
                              aria-label={t("common.selectModel", { defaultValue: "选择模型" })}
                            >
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        <div className="flex items-center h-9 gap-2">
                          <Switch
                            ...
                            aria-label={t("openclaw.reasoning", { defaultValue: "推理模式" })}
                          />
        ...
        <p className="text-xs text-muted-foreground">
          {t("openclaw.modelsHint", {
            defaultValue:
              "配置该模型支持的模型。第一个模型为默认模型（Primary），其余为回退模型（Fallback）。拖拽或调整顺序可更改默认模型。",
          })}
        </p>
      </div>
    </>
