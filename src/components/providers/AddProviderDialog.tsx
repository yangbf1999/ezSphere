...
export function AddProviderDialog({
  ...
  const resolvedPresetId = useMemo(() => {
    ...
    const index = providerPresets.indexOf(initialPreset);
    return index >= 0 ? `claude-${index}` : null;
  }, [initialPresetId, initialPreset]);

  const formKey = `${appId}-${resolvedPresetId ?? "custom"}`;
  ...
  const handleSubmit = useCallback(
    ...
    [appId, onSubmit, handleOpenChange],
  );
  ...
  const providerForm = (
    <ProviderForm
      ...
      showButtons={false}
      initialPresetId={resolvedPresetId}
        ...
        >
              ...
              >
                <TabsList className="mb-4 grid w-full grid-cols-2">
                  <TabsTrigger value="app-specific">
                    {t(`apps.${appId}`)} {t(fromModelsCenter ? "provider.tabModel" : "provider.tabProvider")}
                  </TabsTrigger>
          ...
          <DialogFooter>
            {!showUniversalTab || activeTab === "app-specific" ? (
              <>
                {/*<span className="mr-auto min-w-0 truncate text-xs text-[var(--shell-text-muted)]">
