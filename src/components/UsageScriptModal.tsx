...
const UsageScriptModal: React.FC<UsageScriptModalProps> = ({
  ...
  // 优先级：脚本配置中的显式非空值优先（旧配置可能携带），否则回退模型凭据。
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<string | null>(
    () => {
      ...
      // 官方 CLI/OAuth 模型默认使用官方订阅额度模板，但开关默认关闭
      if (isOfficialSubscription) {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (selectedTemplate === TEMPLATE_TYPES.TOKEN_PLAN) {
        // ZenMux 手填 baseUrl/apiKey；火山是 native 模型，baseUrl 走推理配置，
  ...
  const handleUsePreset = (presetName: string) => {
    ...
    if (preset !== undefined) {
      if (presetName === TEMPLATE_TYPES.CUSTOM) {
        ...
        // 都会在后端回退到模型配置（Provider::resolve_usage_credentials），
  ...
  return (
    <>
      ...
      >
        ...
        >
          <DialogHeader>
            <Gauge className="ez-modal-icon" aria-hidden="true" />
            <DialogTitle>
          ...
          <DialogBody
            className={cn("space-y-6", !script.enabled && "min-h-[320px]")}
          >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1.5">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-primary hover:text-primary/90 transition-colors break-all align-baseline underline-offset-2 hover:underline"
                      >
