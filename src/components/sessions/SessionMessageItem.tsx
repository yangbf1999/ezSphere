...
export const SessionMessageItem = memo(function SessionMessageItem({
  ...
  const roleLabel = getRoleLabel(message.role, t);
  const avatarLetter = isUser
    ? "U"
    : roleLabel.slice(0, 1).toUpperCase() || "A";
    ...
    return (
      ...
      >
        <div className="who">
          ...
          {roleLabel}
          {message.ts ? (
            <span className="msg-ts">{formatTimestamp(message.ts)}</span>
          ) : null}
          <Button
            variant="ghost"
            size="icon"
            ...
            aria-label={t("sessionManager.copyMessage", { defaultValue: "复制内容" })}
          >
