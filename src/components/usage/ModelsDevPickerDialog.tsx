...
interface ModelsDevEntry {
  /** providerId/modelId，同一模型可能出现在多个模型下 */
  key: string;
  providerId: string;
  providerName: string;
  modelId: string;
  ...
  return (
    ...
    >
      <DialogContent
        zIndex="top"
        size="lg"
        className="[&_.ez-modal-content__inner]:h-[80vh]"
        onEscapeKeyDown={(e) => {
      ...
      >
        ...
        <DialogBody className="flex min-h-0 flex-col gap-3 overflow-y-auto py-4">
          {isLoading ? (
                ...
                >
                  ...
                  <SelectContent className="z-[120] max-h-[min(24rem,var(--radix-select-content-available-height))]">
                    <SelectItem value="all">
                      {t("usage.modelsDevAllProviders", "全部模型")}
                    </SelectItem>
