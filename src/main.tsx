...
async function handleConfigLoadError(
  ...
  const path = payload?.path ?? "~/.ezsphere/settings.json";
...
async function bootstrap() {
  ...
  try {
    const initError = isTauriRuntime()
      ? ((await invoke("get_init_error")) as ConfigLoadErrorPayload | null)
      : null;
    if (initError && initError.kind === "db_version_too_new") {
      // 数据库版本过新：渲染应用内「升级应用」恢复界面，不进入正常 App
      ReactDOM.createRoot(document.getElementById("root")!).render(
        <React.StrictMode>
          <ThemeProvider defaultTheme="system" storageKey="ezsphere-theme">
