...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：模型元数据（仅存于 ~/.ezsphere/settings.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
export interface UsageScript {
  ...
  codingPlanProvider?: string; // Coding Plan 模型标识（如 "kimi", "zhipu", "minimax"）
...
// 存储在本地 ~/.ezsphere/settings.json，不随数据库同步
export interface Settings {
  ...
  // ===== 当前模型 ID（设备级）=====
  // 当前 Claude 模型 ID（优先于数据库 is_current）
  currentProviderClaude?: string;
  // 当前 Claude Desktop 模型 ID（优先于数据库 is_current）
  currentProviderClaudeDesktop?: string;
  // 当前 Codex 模型 ID（优先于数据库 is_current）
  currentProviderCodex?: string;
  // 当前 Gemini 模型 ID（优先于数据库 is_current）
...
// ============================================================================
// 统一API网关（Universal API Gateway）- 跨应用共享配置
// ============================================================================
...
// 统一API网关映射（id -> UniversalProvider）
...
// OpenCode 模型配置（settings_config 结构）
...
// OpenClaw 模型配置（settings_config 结构）
...
export interface OpenClawProviderConfig {
  ...
  authHeader?: boolean; // 模型自定义认证开关（如 Longcat）
}
