...
async function fetchProvidersForApp(
  appId: AppId,
...
export const useModelsProvidersQuery = (
  visibleApps?: Partial<Record<AppId, boolean>>,
  options?: UseProvidersQueryOptions,
): { entries: ModelsProviderEntry[]; isLoading: boolean } => {
  const { isProxyRunning = false } = options || {};
  const tauriAvailable = isTauriRuntime();
  const appIds = useMemo(
    () => getVisibleModelsAppIds(visibleApps),
    [visibleApps],
  );
  ...
  const entries = useMemo(() => {
    const result: ModelsProviderEntry[] = [];
    ...
    appIds.forEach((appId, index) => {
      ...
      Object.values(data.providers).forEach((provider) => {
        if (isUniversalSyncedProviderId(provider.id)) return;
        result.push({
          provider,
          appId,
          isCurrent: provider.id === data.currentProviderId,
        });
      });
    });
    ...
    return result.sort((a, b) =>
      compareProvidersByCreatedAt(a.provider, b.provider),
    );
  }, [appIds, queries]);
 ...
 * 未知模型、4xx、脚本/解析错误等）必须立即透出——用户改/删凭据后要马上看到，
 * 否则会一直显示过期额度直到窗口结束。
 *
 ...
 *
 * **确定性失败**（鉴权/空 key/未知模型/4xx 等）不仅立即透出，还会**清空 `lastGood`**：
...
export function resolveDisplayUsage(
  ...
  if (raw?.success) {
    ...
    // 确定性失败（鉴权/空 key/未知模型/4xx 等）：旧成功快照已不可信，丢弃它，
...
export const useUsageQuery = (
  ...
  const query = useQuery<UsageResult>({
    ...
    enabled: enabled && !!providerId && isTauriRuntime(),
    refetchInterval:
      autoQueryInterval > 0
...
export const useSessionMessagesQuery = (
  ...
  return useQuery<SessionMessage[]>({
    ...
    enabled: Boolean(providerId && sourcePath) && isTauriRuntime(),
    staleTime: 30 * 1000,
  });
};
