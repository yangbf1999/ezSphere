...
const computeDefaultAppConfigDir = async (): Promise<string | undefined> => {
  try {
    ...
    return await join(home, ".ezsphere");
  } catch (error) {
    console.error(
...
const computeLegacyAppConfigDir = async (): Promise<string | undefined> => {
  try {
    const home = await homeDir();
    return await join(home, ".ezsphere");
  } catch (error) {
    console.error(
      "[useDirectorySettings] Failed to resolve legacy app config dir",
      error,
    );
    return undefined;
  }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const rawOverride = sanitizeDir(overrideRaw ?? undefined);
        const normalizedOverride = samePath(rawOverride, legacyAppConfig)
          ? undefined
          : rawOverride;
