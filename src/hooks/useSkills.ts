...
import { mergeImportedSkills, sortInstalledSkills } from "@/hooks/useSkills.helpers";

// Debounced query invalidation: coalesce rapid toggle operations into a single refetch
let invalidateTimer: ReturnType<typeof setTimeout> | null = null;
function debouncedInvalidateSkills(queryClient: ReturnType<typeof useQueryClient>) {
...
export function useInstalledSkills() {
  return useQuery({
    ...
    queryFn: async () =>
      sortInstalledSkills(await skillsApi.getInstalled()),
    staleTime: Infinity,
  ...
  return useMutation({
    ...
    onSuccess: (installedSkill, _vars, _ctx) => {
      ...
      queryClient.setQueryData<InstalledSkill[]>(
        ["skills", "installed"],
        (oldData) => {
          if (!oldData) return sortInstalledSkills([installedSkill]);
          return sortInstalledSkills([...oldData, installedSkill]);
        },
      );
...
export function useRestoreSkillBackup() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ backupId }: { backupId: string; currentApp?: AppId }) =>
      skillsApi.restoreBackup(backupId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["skills", "installed"] });
      queryClient.invalidateQueries({ queryKey: ["skills", "backups"] });
    },
...
export function useToggleSkillApp() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      debouncedInvalidateSkills(queryClient);
    },
  });
}
...
export function useInstallSkillsFromZip() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ filePath }: { filePath: string; currentApp?: AppId }) =>
      skillsApi.installFromZip(filePath),
    onSuccess: (installedSkills) => {
      ...
      queryClient.setQueryData<InstalledSkill[]>(
        ...
        (oldData) => {
          ...
          return sortInstalledSkills([...oldData, ...installedSkills]);
        },
      );
    },
  });
  ...
  return useMutation({
    ...
    onSuccess: (updatedSkill) => {
      queryClient.setQueryData<InstalledSkill[]>(
        ["skills", "installed"],
        (oldData) => {
          if (!oldData) return sortInstalledSkills([updatedSkill]);
          return sortInstalledSkills(
            oldData.map((s) =>
