      ...
      if (!isProxyRunning && provider.category !== "official") {
        if (isCopilotProvider) {
          proxyRequiredReason = t("notifications.proxyReasonCopilot", {
            defaultValue: "使用 GitHub Copilot 作为 Claude 模型",
          });
        } else if (
          provider.meta?.apiFormat === "openai_chat" &&
          activeApp === "claude"
      ...
      if (proxyRequiredReason) {
        toast.warning(
          t("notifications.proxyRequiredForSwitch", {
            ...
            defaultValue:
              "此模型{{reason}}，需要代理服务才能正常使用，请先启动代理",
          }),
        );
      }
      ...
      if (isProxyTakeover && provider.category === "official") {
        toast.error(
          t("notifications.officialBlockedByProxy", {
            defaultValue:
              "代理接管模式下不能切换到官方模型，使用代理访问官方 API 可能导致账号被封禁",
          }),
          { duration: 6000 },
        );
        return;
        ...
        if (!proxyRequiredReason) {
          ...
          if (activeApp === "codex") {
            ...
            if (provider.meta?.claudeDesktopMode === "proxy") {
              ...
              defaultMessage =
                "切换成功，请保持 ezSphere 运行，并重启 Claude Desktop 后生效";
            } else {
