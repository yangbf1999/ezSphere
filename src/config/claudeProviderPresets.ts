 ...
 * 预设模型配置模板
 *
 * 模型中心目录展示范围见 vendorPresets.ts MODELS_DIRECTORY_VENDOR_NAMES；
 * 不在清单内的预设已标记 hidden: true（保留配置供测试与后续恢复）。
 */
...
export interface ProviderPreset {
  ...
  // 模型类型标识（用于特殊模型检测）
  // - "github_copilot": GitHub Copilot 模型（需要 OAuth 认证）
...
export const providerPresets: ProviderPreset[] = [
  ...
  },
  {
    ...
    websiteUrl: "https://cloud.baidu.com/product-s/qianfan_home?track=home_banner",
    apiKeyUrl:
  ...
  },
  {
    ...
    // 说明：该模型使用 ANTHROPIC_API_KEY（而非 ANTHROPIC_AUTH_TOKEN）
    apiKeyField: "ANTHROPIC_API_KEY",
    settingsConfig: {
      env: {
  ...
  },
  {
    ...
    apiKeyUrl: "https://www.crazyrouter.com/register?aff=OZcm&ref=ezSphere",
    settingsConfig: {
      env: {
